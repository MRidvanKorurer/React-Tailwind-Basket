...
const Productshow = ({item}) => {
  ...
  const handleClickAddToBasket = () => {
    setCount(prevState => [...prevState, item]);
  }
  ...
  <div className="relative border border-gray-100 bg-white p-6">
    ...
    <p className="mt-1.5 text-lg text-gray-700">${item.price}</p>

    <form className="mt-4">
      <button
        onClick={handleClickAddToBasket}
        className="block w-full rounded bg-indigo-600 text-white p-4 text-sm font-medium transition hover:scale-105"
      >
        Sepete Ekle
      </button>
    </form>
